...
namespace fAI
{
    ...
    {
        private static string GetLanguageJsonFile()
        {
            var p = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            return Path.Combine(p, @"TextToSpeech\Languages", "Languages.json");
        }

        public static Languages Load()
        {
            return JsonConvert.DeserializeObject<Languages>(LANGUAGES_DEFINITION_JSON);
        }

        // https://github.com/polterguy/babelfish.frontend/blob/master/src/app/languages.json
        const string LANGUAGES_DEFINITION_JSON = @"[
  {
    ""code"": ""aa"",
    ""name"": ""Afar"",
    ""native"": ""Afar""
