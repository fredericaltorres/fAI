...
namespace fAI
{
    ...
    public class GenericAISpeech : HttpBase, IGenericAISpeech
    {
        ...
        public List<TTSRequest>  TTSVoiceInfos = new List<TTSRequest>
            {
                new TTSRequest {  Model = "mistralai/voxtral-mini-tts-2603", Voices = new List<string>() {
            ...
            int inputTokenCount = -1)
        {
            ...
            if (response.Success)
            {
                ...
                sw.Stop();
                OpenAI.Trace($"[TTS] Model: {model}, Duration: {sw.ElapsedMilliseconds} ms, mp3FileName: ({mp3FileName})", this);
                return mp3FileName;
            }
