...
namespace fAI
{
    public class GenericAISpeech : HttpBase, IGenericAISpeech
    {
        ...
        public class TTSRequest
        {
            ...
            public bool SupportVoiceCloning { get; set; }
        ...
        public List<TTSRequest> TTSVoiceInfos = new List<TTSRequest>
        {
            ...
            // curl.exe "https://api.fish.audio/model?self=true&page_size=20" --header "Authorization: Bearer sk-fish-CE5N2CCRysrjZBX8d1vfuQTvsqOzC5BV9U5n6Twa8m8"
            // https://docs.fish.audio/features/text-to-speech
            // https://docs.fish.audio/features/voice-cloning
            new TTSRequest { PricePerMillionOfChars=15,     Model = "fish-audio/s2.1-pro",
                Voices = new List<string>() {
                    "bf322df2096a46f18c579d0baa36f41d", // Adrian, usa
                    "b347db033a6549378b48d00acb0d06cd",  // Selene, usa
                    "933563129e564b19a115bedd57b7406a", // Sarah, usa
                    "536d3a5e000945adb7038665781a4aca", // Ethan, usa
                    "e3cd384158934cc9a01029cd7d278634", // Laura, usa
                    "79d0bd3e4e5444b18f7b6d89b5927bf1", // Jordan, usa
                    "9a9cf47702da476aa4629e2506d4a857", // Hannah, usa
            ...
            )
        {
            ...
            dynamic payload = inputVoiceFileName != null ? GetPayLoadWithInputVoiceModel(input, voice, model, inputVoiceFileName, inputVoiceText) : GetPayLoad(input, voice, model);

            var wc = InitWebClient();
            var response = wc.POST(__url, payload);
            if (response.Success)
            {
        ...
        private string GetPayLoad(string input, string voice, string model, string response_format = "mp3", float speed = 1f)
        {
            return JsonConvert.SerializeObject(new
            {
                input,
                model,
                response_format = model.Contains("google") ? "pcm" : response_format,
                speed,
                voice
            });
        }

        private string GetPayLoadWithInputVoiceModel(
            string input, string voice, string model, 
            string inputVoiceFileName,
            string inputVoiceText,
            string response_format = "mp3", float speed = 1f)
        {
            return JsonConvert.SerializeObject(new
            {
                input,
                ...
                speed,
                //voice,
                input_references = new List<dynamic> { 
                    new { 
                        text = inputVoiceText, 
                        type = "text" 
                    },
                    new { 
                        input_audio = new { data= "data:audio/wav;base64,"+FileUtil.FileToBase64(inputVoiceFileName) },
                        type = "input_audio"
                    }
                }
            });
