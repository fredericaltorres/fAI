...
namespace fAI.Tests
{
    ...
    public class RunTimeAnalysisTests : OpenAIUnitTestsBase
    {
        ...
        {
            var exceptionDescriptionFileName = ExceptionAnalyzer.RunCode(new S.Action(() =>
            {
                var result = new RunTimeAnalysis_Case1().Run(1); // This will throw a DivideByZeroException
            }));

            var ea = ExceptionAnalyzer.Load(exceptionDescriptionFileName);

            var analysisReportFileName = ea.AnalyzeAndGenerateReport(new Prompt_GPT_4o());
        }
        ...
        public void RunTimeAnalysis_DivisionByZero_MissingInitializationInConfigFile()
        {
            var exceptionDescriptionFileName = ExceptionAnalyzer.RunCode(new S.Action(() =>
            {
                var result = new RunTimeAnalysis_Case2().Run(1);
